...
var cycleNotes = 1;
var bassCycle = 10;
var sharpNoteCycle = 7;
var bassSharpCycle = 0;
var chordCycle = 1;
var songCycle = 1;
var levelProgression = 1;
var correctNoteHit = Boolean(false);
var userNoteProgression = {};
var gameProgression = {};
var numberOfNotesInLevel = 0;
var noteNumberInGame = 0;
var resetValues = Boolean(true);
var missedNote = Boolean(true);
var previousNote = null;
var deductionReason;
var deductionCode;
var scoreMultiplier = 1;
// Class for animated notes
var animatingNotes = /*#__PURE__*/function () {
  function animatingNotes(x, y, yHit, y1, y1Hit, y2, y2Hit, major, major1, major2) {
    _classCallCheck(this, animatingNotes);
    this.x = x;
    this.y = y;
    this.yHit = yHit;
    this.y1 = y1;
  ...
  _createClass(animatingNotes, [{
    key: "update",
    value: function update() {
      // Updates and changes each animated notes x variable according to speed
      if (toggle) {
        if (this.x <= leftBoundary + window.innerWidth * DynamicDifficulty[difficultyLevel].hitScreenPercentage) {
          if (this.recordedExpectedTime == false) {
            ...
            var ExpectedTime = new Date(); // Creates a time that the note will reach the desired hit point
            ...
            if (noteNumberInGame > 1 && resetValues == true) {
              // Needs to be reset so it does not record when the first 2 notes are generated
              resetValues = false;
              noteNumberInGame = 0;
        ...
        if (pressed) {
          if (this.x > lowerBoundary && this.x < upperBoundary) {
            if (!keyPressed) {
              // Stops constantly adding to score whilst holding note
              keyPressed = true;
              var hitTime = new Date(); // Creates a time that the user pressed a note
              if (notesHeldList.length == DynamicDifficulty[difficultyLevel].numberOfNotes) {
                var _notesHeldListValue;
                for (var values in notesHeldList) {
                  if (majorKeyPos.includes(findNoteLetter(notesHeldList[values]))) {
                    var notesHeldOctave = parseInt(notesHeldList[values] / 12) - 4;
                    var notesHeldLetter = findNoteLetter(notesHeldList[values]);
                    var notesHeldValue = staffShtMajorPos.indexOf(notesHeldLetter);
                    if (notesHeldOctave === 1) {
                      _notesHeldListValue = notesHeldValue * 0.5 + 4;
                    } else if (notesHeldOctave === 0) {
                      _notesHeldListValue = notesHeldValue * 0.5 + 4 + 3.5;
                    } else if (notesHeldOctave === 2) {
                      _notesHeldListValue = notesHeldValue * 0.5 + 4 - 3.5;
                    } else _notesHeldListValue = 0;
                  } else if (sharpKeyPos.includes(findNoteLetter(notesHeldList[values]))) {
                    var _notesHeldOctave = parseInt(notesHeldList[values] / 12) - 4;
                    var _notesHeldLetter = findNoteLetter(notesHeldList[values]);
                    var _notesHeldValue = staffShtSharpPos.indexOf(_notesHeldLetter);
                    if (_notesHeldOctave === 1) {
                      _notesHeldListValue = _notesHeldValue * 0.5 + 4;
                    } else if (_notesHeldOctave === 0) {
                      _notesHeldListValue = _notesHeldValue * 0.5 + 4 + 3.5;
                    } else if (_notesHeldOctave === 2) {
                      _notesHeldListValue = _notesHeldValue * 0.5 + 4 - 3.5;
                    } else _notesHeldListValue = 0;
                  }
                  if (this.y / staffSpacing + 0.5 == _notesHeldListValue && this.major == majorPressed) {
                    this.yHit = true;
                  }
                  if (this.y1 / staffSpacing + 0.5 == _notesHeldListValue && this.major == majorPressed) {
                    this.y1Hit = true;
                  }
                  if (this.y2 / staffSpacing + 0.5 == _notesHeldListValue && this.major == majorPressed) {
                    this.y2Hit = true;
                  }
                ...
                if (notesHeldList.length == DynamicDifficulty[difficultyLevel].numberOfNotes - 1 && this.y1Hit == true) {
                  for (var value in notesHeldList) {
                    if (majorKeyPos.includes(findNoteLetter(notesHeldList[value]))) {
                      var _notesHeldOctave2 = parseInt(notesHeldList[value] / 12) - 4;
                      var _notesHeldLetter2 = findNoteLetter(notesHeldList[value]);
                      var _notesHeldValue2 = staffShtMajorPos.indexOf(_notesHeldLetter2);
                      if (_notesHeldOctave2 === 1) {
                        notesHeldListValue = _notesHeldValue2 * 0.5 + 4;
                      } else if (_notesHeldOctave2 === 0) {
                        notesHeldListValue = _notesHeldValue2 * 0.5 + 4 + 3.5;
                      } else if (_notesHeldOctave2 === 2) {
                        notesHeldListValue = _notesHeldValue2 * 0.5 + 4 - 3.5;
                      } else notesHeldListValue = 0;
                    } else if (sharpKeyPos.includes(findNoteLetter(notesHeldList[value]))) {
                      var _notesHeldOctave3 = parseInt(notesHeldList[value] / 12) - 4;
                      var _notesHeldLetter3 = findNoteLetter(notesHeldList[value]);
                      var _notesHeldValue3 = staffShtSharpPos.indexOf(_notesHeldLetter3);
                      if (_notesHeldOctave3 === 1) {
                        notesHeldListValue = _notesHeldValue3 * 0.5 + 4;
                      } else if (_notesHeldOctave3 === 0) {
                        notesHeldListValue = _notesHeldValue3 * 0.5 + 4 + 3.5;
                      } else if (_notesHeldOctave3 === 2) {
                        notesHeldListValue = _notesHeldValue3 * 0.5 + 4 - 3.5;
                      } else notesHeldListValue = 0;
                    }
              ...
              } else if (this.doNotDeductScore == false) {
                // Wrong number of notes is pushed
            ...
            if (this.scoreAdjusted == false && resetValues == false && this.x < hitMarker && this.doNotDeductScore == false) {
              ...
              var missedTime = new Date(); // Creates a time that the user pressed a note
              score--;
        ...
        if (this.x < leftBoundary) {
          // If note oves far to the left it will delete and autocreate a new y value
          if (this.scoreAdjusted == false && resetValues == false && this.doNotDeductScore == false) {
            // Missed the note
            score--;
          ...
          if (resetValues == false) {
            ...
            if (numberOfNotesInLevel > 0) {
              // Otherwise distanceBetweenPreviousNote will error
          ...
          switch (difficultyLevel) {
            case 1:
              ...
              break;
            case 2:
              // roll up and down 7 notes to teach moving fingers correctly
              this.y = staffSpacing * cycleNotes;
              this.major = true;
              if (DynamicDifficulty[2].down == true) {
                // Scrolls up and down through the octave
            ...
            case 4:
              ...
              this.y = staffSpacing * cycleNotes;
              if (DynamicDifficulty[4].down == true) {
                // Scrolls up and down through the octave
              ...
              break;
            case 5:
              // random notes including sharps
              var trebleSharpValue;
            ...
            case 9:
              ...
              this.major = true;
              if (DynamicDifficulty[9].down == false) {
                // Scrolls up and down through the octave
            ...
            case 10:
              ...
              break;
            case 11:
              // roll up and down including sharps
              this.y = staffSpacing * bassCycle;
              if (DynamicDifficulty[11].down == false) {
                // Scrolls up and down through the octave
            ...
            case 13:
              ...
              break;
            case 14:
              // 3 note chords in different order
              if (bassChords[chordCycle][1][1] == "major") {
                this.image = noteImage;
                this.major = true;
              } else if (bassChords[chordCycle][1][1] == "sharp") {
    ...
    value: function display() {
      // Displays the notes based on the x and y values created in the update function
